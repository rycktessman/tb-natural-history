...
#generate data frame where each row is a timestep, each column is a state (track proportion of pop by state)
...
nat_hist_micro <- function(pop, r, p)  { #inputs = sim_pop row, random numbers subset, params
  curr_pop <- (pop==1)*(
    2*(r$tb_to_tb_m==1) +
      3*(r$tb_to_tb_s==1) +
      5*(r$tb_to_sp_cure==1) +
...
#CLEANING FUNCTION (apply to both markov and individual versions)
