...
targets_prev <- read_xlsx("data/country_targets.xlsx", range="A1:G1000")[,c(1,7)]
...
prev_cases <- 278 #
m_cases <- 108 #info on current/tx history not given
s_cases <- 106 #info on current/tx history not given
ms_cases <- 54 #% of all smear+ that were symptomatic (slightly different but close to # of smear+ cases)
...
prev_lb <- 244*(1-5/278)/100000 #estimated prev from survey, adjusting for 5/278 currently on tx 
prev_ub <- 330*(1-5/278)/100000 #estimated prev from survey, adjusting for 5/278 currently on tx 
...
prev_samples <- rbinom(n=100000, size=60000, prob=prev)/60000 #sample size chosen to match lb and ub
...
prop_clindx_tb_samples <- rbeta(n=100000, shape1=2.5, shape2=7.5) #matches mean and lower bound above
notif_samples <- notif_labconf + notif_clindx*prop_clindx_tb_samples 
...
#treated CFR - use normal distribution since less skew then other countries
cfr_samples <- rnorm(n=100000, mean=targets_prev %>% filter(param=="treat_tb_cfr") %>% pull(value), 
                      sd=0.015)
...
m_ac_annual <- 0.00389 #all-cause/non-TB mortality probability (applied to symptom- TB too - annual avg. mortality probability among 15-64 years in Cambodia)
...
save(n, verbose, cyc_len, params, names_params_calib, names_params_calib_mult,
     ...
     file="data/params_targets_bangladesh.Rda")